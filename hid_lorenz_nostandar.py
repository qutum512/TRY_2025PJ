...
# =================== Dataset ===================
def build_dataset_xy_windows(data_xy, seq_len, dt):
    """
    物理域下的 (x,y)，不做标准化。
    输出：
      X: (N, 2*seq_len) 以 [x1,y1,x2,y2,...,xL,yL] 展平
      y_deriv: (N, 2) 中心差分的 [dx/dt, dy/dt]
      xy_tail_for_rollout: (1, 2, seq_len) 末段历史窗口（物理域）
      idxs: 对应中心索引
    """
    Ttot = data_xy.shape[0]
    i_start = seq_len - 1
    i_end = Ttot - 2
    ...
    for k, i in enumerate(idxs):
        win = data_xy[i-seq_len+1:i+1, :]          # (seq_len, 2)
        X[k] = win.reshape(-1)                      # [x1,y1,x2,y2,...,xL,yL]
        dx_i = (x[i+1] - x[i-1]) / (2.0*dt)
        dy_i = (y[i+1] - y[i-1]) / (2.0*dt)
        y_deriv[k, 0] = dx_i
        y_deriv[k, 1] = dy_i

    tail = data_xy[-seq_len:, :].T.reshape(1, 2, seq_len).astype(np.float32)
...
class MLP_z_only(nn.Module):
    def __init__(self, seq_len, hidden=256, layers=3, z_scale=40.0):
        super().__init__()
        in_dim = 2 * seq_len
        h = hidden
    ...
    def forward(self, x_flat):
        h = self.net(x_flat)
        raw = self.head_z(h)
        return self.z_scale * torch.tanh(raw / self.z_scale)
...
@torch.no_grad()
def rollout_future_mlp_global(xy_hist, model_z, coeff, dt, steps, device):
    """
    输入/输出均在物理域（不标准化）。
    """
    cur = xy_hist.clone().to(device)          # (1, 2, seq_len)
    preds = []
    for _ in range(steps):
        x_t = cur[:, 0, -1:].view(1, 1)
        y_t = cur[:, 1, -1:].view(1, 1)
        # 展平顺序与构建数据集一致：[x1,y1,x2,y2,...,xL,yL]
        x_flat = cur.permute(0, 2, 1).reshape(1, -1)   
        z_hat = model_z(x_flat)                        # (1,1)
...
def main():
    ...
    # ---- 构建数据集（无标准化）----
    X, y_deriv, xy_tail_rollout, idxs = build_dataset_xy_windows(data_xy, seq_len, dt)
    N = X.shape[0]
    ...
    # ---- 模型、系数、优化器 ----
    model_z = MLP_z_only(seq_len, hidden=mlp_hidden, layers=mlp_layers, z_scale=z_scale).to(device)
    coeff = GlobalQuadCoeff().to(device)
    opt = optim.Adam(
        [
            {"params": model_z.parameters(), "lr": lr},
            {"params": coeff.parameters(),  "lr": lr * coeff_lr_scale},
        ],
    ...
    for ep in range(1, epochs + 1):
        ...
        tot_loss = tot_x = tot_y = 0.0
        for b in range(num_batches):
            idx = perm[b*batch_size:(b+1)*batch_size]
            xb, yb = X_t[idx], y_t[idx]
            x_last, y_last = xb[:, -2], xb[:, -1]      # 对应窗口最后的 (x_L, y_L)
            z_hat = model_z(xb)
            ...
            loss_x = loss_fn(rhs_x, dx_tgt)
            loss_y = loss_fn(rhs_y, dy_tgt)
            loss = loss_x + loss_y

            opt.zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(list(model_z.parameters()) + list(coeff.parameters()), grad_clip)
            opt.step()
    ...
    model_z.eval()
    with torch.no_grad():
        preds = rollout_future_mlp_global(xy_tail_rollout, model_z, coeff, dt, future_steps, device)
    ...
    hist_xy = data_xy[-seq_len:, :]

    # 1) timeseries
    fig = plt.figure(figsize=(13,7)); ax = fig.add_subplot(111)
    ax.plot(t_hist, hist_xy[:,0], label="History x")
    ax.plot(t_hist, hist_xy[:,1], label="History y")
    ax.plot(t_future, true_future[:,0], label="True future x")
    ax.plot(t_future, true_future[:,1], label="True future y")
    ax.plot(t_future, preds[:,0], '--', label="Pred x")
    ax.plot(t_future, preds[:,1], '--', label="Pred y")
    ...
    ax.plot(hist_xy[:,0], hist_xy[:,1], label="History")
    ax.plot(true_future[:,0], true_future[:,1], label="True")
    ax.plot(preds[:,0], preds[:,1], '--', label="Pred")
    ax.legend(); ax.grid()
    ...
    axes[1].scatter(true_future[:,0], preds[:,0], s=10, alpha=0.7, label="x")
    axes[1].scatter(true_future[:,1], preds[:,1], s=10, alpha=0.7, label="y")
    minv, maxv = min(true_future.min(), preds.min()), max(true_future.max(), preds.max())
